...
function updateEnemyLaser($container){
  ...
  for(let i = 0; i < enemyLasers.length; i++){
    ...
    if (enemyLaser.y > GAME_HEIGHT-30){
      ...
      if(score>hiscoreval){
        if(flag){
          ...
          localStorage.setItem("pro",userName);
          flag = false;
        }
        
        hiscoreval = score;
        localStorage.setItem("hiscore", JSON.stringify(hiscoreval));
        let me = localStorage.getItem("pro")
        hiscoreBox.innerHTML = "High Score: " + hiscoreval + " " + me;
      }
    ...
    if(collideRect(spaceship_rectangle, enemyLaser_rectangle)){
      ...
      localStorage.setItem("lifey",life);
      let lineval = localStorage.getItem("lifey");
      lifeLine.innerHTML = lineval;
      if(lineval>0){
        const $container = document.querySelector(".main");
      }
      else{
        STATE.gameOver = true;
        createEnemyLaser = false;
...
if(hiscore === null){
    ...
    let me = localStorage.getItem("pro")
    hiscoreBox.innerHTML = "High Score: " + hiscore + " " + me;
}
